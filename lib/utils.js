...
function rootKey (key) {
  if (!key) throw new Error('root key is required')
  if (ROOT_KEYS_ABREV.has(key)) return ROOT_KEYS_ABREV.get(key)
  if (!ROOT_KEYS.has(key)) throw new Error('invalid root key')
  return key
}
